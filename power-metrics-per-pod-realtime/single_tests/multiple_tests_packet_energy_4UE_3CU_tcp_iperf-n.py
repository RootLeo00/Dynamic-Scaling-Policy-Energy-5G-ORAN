...
def deploy_bp3_with_second_du_and_ue(cpu_mi=None, memory_mi='2Gi', tcpdump=False):
    ...
    wait_for_pods(RAN_NAMESPACE)
    run_command(["helm", "install", "oai-du3", DU3_CHART_PATH, "-n", RAN_NAMESPACE, "--set", "config.cuHost=oai-cu3.ran.svc.cluster.local"])
    wait_for_pods(RAN_NAMESPACE)
    ...
    # Install the second DU and UE
    run_command(["helm", "install", "oai-du4", DU4_CHART_PATH, "-n", RAN_NAMESPACE, "--set", "config.cuHost=oai-cu3.ran.svc.cluster.local"])
    wait_for_pods(RAN_NAMESPACE)
...
def run_experiments_with_multiple_ues(index):
    ...
    for packet_length in packet_lengths:
        for ue1_mb, ue2_mb, ue3_mb, ue4_mb in zip(ue1_mb_values, ue2_mb_values, ue3_mb_values, ue4_mb_values):
            ...
            experiment_dir = f"experiment_packet_energy_4UE_3CU_iperf-n_{index}/{total_mb}_{packet_length}"
